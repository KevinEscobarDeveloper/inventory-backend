	...
	public ResponseEntity<CategoryResponseRest> searchById(Long id) {
		...
		try {
			...
			if(category.isPresent()) {
				...
				response.setMetadata("Respuesta ok", "001", "Categoria encontrada");
			} else {
				response.setMetadata("Error", "-1", "Error Categoria no encontrada");
		...
		try {
			...
			if(categorySearch.isPresent()) {
				...
				if(categoryToUpdate != null) {
					...
					response.getCategoryResponse().setCategory(list);
					response.setMetadata("Respuesta ok", "001", "Categoria actualizada");
				} else {
					response.setMetadata("Error", "-1", "Error al Categoria no actualizada");
					return new ResponseEntity<CategoryResponseRest>(response,HttpStatus.BAD_REQUEST);
				}
			} else {
				response.setMetadata("Error", "-1", "Error al Categoria no encontrada");
				return new ResponseEntity<CategoryResponseRest>(response,HttpStatus.NOT_FOUND);
			}
		}catch(Exception e) {
